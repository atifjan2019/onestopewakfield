...
export default function ContactForm() {
  ...
  if (status === 'success') {
    return (
      <div className="bg-white p-8 md:p-10 rounded-3xl border border-black/5 shadow-sm h-full flex flex-col items-center justify-center text-center">
        ...
        <h3 className="text-2xl font-black text-text mb-2">Message Sent.</h3>
        ...
        <button onClick={() => setStatus('idle')} className="mt-8 px-6 py-3 bg-surface hover:bg-accent/5 text-text font-bold rounded-xl transition-colors border border-black/8">
          Send Another
        </button>
      </div>
    );
  ...
  return (
    <div className="bg-white p-8 md:p-10 rounded-3xl border border-black/5 shadow-sm relative overflow-hidden">
      <div className="absolute top-0 right-0 w-64 h-64 bg-accent/3 rounded-full blur-[80px] pointer-events-none"></div>
      
      <h2 className="text-2xl font-black text-text mb-2 relative z-10">Send a Message</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="block text-[11px] font-black tracking-widest text-text-muted uppercase pl-1">Email <span className="text-black/30 font-medium normal-case">(Optional)</span></label>
            <input 
              type="email" 
              className="w-full bg-surface border border-black/8 rounded-2xl px-5 py-4 text-text hover:border-accent/40 focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all font-bold placeholder:text-black/20"
        ...
        <div className="space-y-2">
          ...
          <textarea 
            required rows={4}
            className="w-full bg-surface border border-black/8 rounded-2xl px-5 py-4 text-text hover:border-accent/40 focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all font-bold placeholder:text-black/20 resize-none"
