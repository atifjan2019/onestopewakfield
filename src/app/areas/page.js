...
export default function AreasPage() {
  ...
  return (
    <div className="bg-primary min-h-screen pt-20">
      ...
      {/* Hero */}
      <section className="relative py-8 md:py-16 px-4 md:px-6 overflow-hidden flex flex-col items-center justify-center text-center">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {areas.map((area) => (
              <Link
                ...
                className="glass-panel text-center text-white hover:text-accent font-bold px-4 py-4 rounded-xl transition-all duration-300 hover:-translate-y-1 hover:border-accent/40 hover:bg-accent/5 shadow-sm"
              >
                {area.name}
              </Link>
            ))}
